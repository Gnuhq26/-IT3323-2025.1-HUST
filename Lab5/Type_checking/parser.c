...
void compileForSt(void)
{
  ...
  // The controller and bounds must be basic types (INTEGER or CHAR)
  checkBasicType(varType);

  eat(SB_ASSIGN);
  type1 = compileExpression();
  // exp1 must have same basic type as the control variable
  checkTypeEquality(varType, type1);

  eat(KW_TO);
  type2 = compileExpression();
  // exp2 must have same basic type as the control variable
